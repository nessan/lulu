...
It should return true if the first one is considered dominant and false otherwise.
...
You can explicitly set the `comparator` argument to `false` to disable key sorting.
...
1. You can use Lua function like `table.keys(tbl, function(a,b) return a > b end)`.
