...
`table.metadata(tbl)`            | Returns a table of metadata about `tbl`.
...
`md[t].refs`                  | Number of references to `t`. Greater than 1 if `t` is shared.
: {.bordered .striped .hover .responsive tbl-colwidths="[30,70]"}

This metadata is useful for understanding the structure of a table.
