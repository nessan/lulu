...
All you get is another *pointer* to the same block of memory.
...
putln("shallow copy = %t", shallow)
```
1. Here, we have somewhat spuriously introduced sub-tables instead of setting `orig.a = "alpha"` we have `orig.a[1] = "alpha"`etc.
2. Deep copies are entirely independent, so this change will only affect `deep`.
...
2. In contrast, a change to a sub-element is seen in the shallow copy and also in the original table.
