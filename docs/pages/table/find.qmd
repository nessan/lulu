...
Returns `true` if there is a top-level value in `tbl` that is *identical in content* to `value`.
...
The matches are done using {table.eq} so, if the hunted-for object is a table, we look for one identical in content, not just occupying the same memory blocks.
...
On failure, it returns the trio `nil, nil, nil`.
...
```
If you run this multiple times, you will get the trio: bar 43 true or baba 44 true.
