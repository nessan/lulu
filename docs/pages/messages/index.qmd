...
These are used throughout `lulu`.
...
If you're writing a library, it is often helpful to have the source code location of the error point **up** the stack to where the library *user* made an erroneous call.
...
It would be preferable to point instead to the source code location where the library's caller made the error.
...
NOTE: The module's `source_info` method makes use of Lua's introspection facilities and, in particular, [`debug.getinfo`](https://www.lua.org/pil/23.1.html).
