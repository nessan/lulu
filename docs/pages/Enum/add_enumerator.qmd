...
We use this feature when we are using `lulu` to create domain specific languages and want to add an enumerator for a new domain.
...
If the second argument is a table we add an enumerator to the `Enum` instance with the specified associated data.
The ordinal value can be specified in the table of associated data under the `ordinal` field or it will be automatically generated as the next consecutive integer in the existing sequence.
...
```
1. The ordinal value in the associated data is ignored as we specified the value in the second argument.
2. The ordinal value in the associated data is ignored as we specified the value in the second argument.
